	...
	if (op.security && options.getAuthorization) {
		return op.security.reduce<Promise<api.OperationRights>>((promise, security) => {
			return promise.then(rights => {
				...
				return options.getAuthorization(security, securityDefinition, op).then(auth => {
...
function makeFetchRequest(req: api.ServiceRequest): Promise<Response> {
	...
	if (options.fetchOptions) {
		...
		const headers = opts.headers ? Object.assign(fetchOptions.headers, opts.headers) : fetchOptions.headers;
...
function buildRequest(
	op: api.OperationInfo,
	baseUrl: string,
	parameters: api.OperationParamGroups,
	rights: api.OperationRights
): api.ServiceRequest {
...
function buildUrl(op: api.OperationInfo, baseUrl: string, parameters, spec: api.OpenApiSpec) {
	...
	if (parameters.path) {
		url = Object.keys(parameters.path).reduce((url, name) => url.replace(`{${name}}`, parameters.path[name]), url);
	}
	...
	Object.keys(rights).forEach(name => {
		...
		switch (definition.type) {
			case 'basic':
				...
				const token =
					typeof window !== 'undefined' && window.btoa ? window.btoa(creds) : new Buffer(creds).toString('base64');
