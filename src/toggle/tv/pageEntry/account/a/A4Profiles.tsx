...
import 'ref/tv/pageEntry/account/a/A4Profiles.scss';
	...
	componentWillReceiveProps(nextProps: A4ProfilesProps) {
		if (nextProps.activeProfile) {
			if (
				...
				(this.activeProfile && this.activeProfile.isRestricted !== nextProps.activeProfile.isRestricted)
			) {
				this.activeProfile = nextProps.activeProfile;
			}
		}
		...
		switch (act) {
			case 'click':
				...
				if (selectedIndex < profiles.length) {
					// Edit profile
					if (profiles[selectedIndex].isRestricted) {
