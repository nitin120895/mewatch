...
import { getColumnClasses, isLessThanTabletSize } from '../../../util/grid';
...
export default class SeasonSelector extends React.Component<SeasonSelectorProps, SeasonSelectorState> {
	...
	private onResize = () => {
		const isMobile = isLessThanTabletSize();
	...
	render() {
		...
		const label =
			selectedSeason.subtype === 'SeasonPrequel' ? subTypeLabels.SeasonPrequel : subTypeLabels.EntertainmentSeason;
		...
		return (
			<div className={className}>
				{isDropdownExpanded && <div className="d1-season-selector-overlay" onClick={this.onClick} />}
