...
import CarouselItem from 'toggle/responsive/pageEntry/hero/common/CarouselItem';
import { isClickToWatch } from 'toggle/responsive/util/item';
...
export default class Carousel extends React.Component<CarouselProps, any> {
	...
	private isSidePanelOverlayDisplay: boolean;
	...
	componentDidMount() {
		...
		document.addEventListener('sideNavVisible', this.sideNavVisibleChange, false);
	}

	componentWillUnmount() {
		this.stopAutoPlay();
		document.removeEventListener('sideNavVisible', this.sideNavVisibleChange);
	...
	private autoPlay = (autoCycle?: number) => {
		if (this.shouldCarouselAutoplay(autoCycle) && !this.autoPlaying) {
			...
			this.autoPlaySetInterval = window.setInterval(() => {
				!this.isSidePanelOverlayDisplay && this.transitionPane('->', this.itemIndex + 1);
	...
	*/
	...
	private sideNavVisibleChange = e => {
		this.isSidePanelOverlayDisplay = e.detail;
	};
	...
	private renderCarouselItem(index: number) {
		...
		const itemExists = viewableItems[index] && viewableItems[index].item;
		if (!itemExists) return;

		const carouselItem = viewableItems[index].item;

		const { imageOffsetTop, template } = this.props;
		const isClickToPlay = isClickToWatch(carouselItem, template);

		const renderItem = () => {
			return (
				<CarouselItem
					key={`item-${carouselItem.path}-${index}`}
		...
		return isClickToPlay ? (
			<CTAWrapper type={CTATypes.Watch} data={{ item: carouselItem }} key={`cta-item-${carouselItem.path}-${index}`}>
				{renderItem()}
			</CTAWrapper>
		) : (
			renderItem()
