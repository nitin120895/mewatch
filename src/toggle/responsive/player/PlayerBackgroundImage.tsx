...
import { isPortraitVideo } from 'toggle/responsive/page/item/itemUtil';
...
> {
	...
	private getImageSources() {
		...
		const imageType = isPortraitVideo(item) && images.tile ? 'tile' : 'wallpaper';
		const mobile = resolveImages(images, imageType, { width: 480 })[0].src;
		const tablet = resolveImages(images, imageType, { width: 1440 })[0].src;
		const desktopWide = resolveImages(images, imageType, { width: 1920 })[0].src;
		this.setState({
			imageSources: [
