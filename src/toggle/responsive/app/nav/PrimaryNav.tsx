...
import { isElementHidden } from 'shared/util/browser';
...
export default class PrimaryNav extends React.PureComponent<PrimaryNavProps, PrimaryNavState> {
	...
	private hideEntries = () => {
		...
		if (!this.nav || isSearchPage) return;
		...
		for (let i = totalEntries - 1; i >= 0; --i) {
			...
			if (element.getBoundingClientRect().top === navRect.top && !isElementHidden(element)) break;
	...
	private onFirstRender = () => {
		if (!this.nav) return;

		// wait a bit while custom Font will be applied on page load to avoid error related to different letter size
		setTimeout(() => window.requestAnimationFrame(this.hideEntries), 100);
	};
	...
	private onLinkClick = () => {
		...
		updateSubscriptionEntryPoint(MixpanelEntryPoint.Menu);
	};
	...
	private renderEntryLink(entry: api.NavEntry, index: number): any {
		...
		return (
			<MenuWrapper type={MenuTypes.Main} menuItemsOrder={[entry.label]}>
				<Link
					className={classes}
					to={path}
					onMouseEnter={onMouseEnter}
		...
		return (
			<DropMenu onDismiss={this.onDismissDropMenu} autoFocus={false}>
				<SecondaryNav
					...
					autoFocus={false}
					shouldHideMeRewardsTitle={true}
					menuType={MenuTypes.HoverDropdown}
					onClickEntry={this.onLinkClick}
					headerNav
				/>
			</DropMenu>
		);
