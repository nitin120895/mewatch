...
export default class ProfileIcon extends React.Component<ProfileIconProps, any> {
	...
	private updateColor({ profile }: ProfileIconProps) {
		if (!this.mounted || !this.circle || this.props.isPrimaryProfile) return;
	...
	renderCircle(): any {
		const { profile, isPrimaryProfile } = this.props;
		if (!profile || !profile.name) return;
		const style = !isPrimaryProfile ? { backgroundColor: profile.color } : undefined;

		const firstCharacter = getFirstCharacter(profile.name);
		return (
			<span key="circle" ref={this.onCircleReference} className={bem.e('circle')} style={style}>
