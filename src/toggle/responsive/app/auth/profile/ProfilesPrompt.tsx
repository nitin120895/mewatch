...
import 'ref/responsive/app/auth/profile/ProfilesPrompt.scss';
			...
			>
				{enabled &&
					profiles.map((p, i) => {
						...
						const buttonLoading = (loading && id === p.id) || (id === p.id && !selectedProfile.isRestricted);
						const isPrimary = account.primaryProfileId === p.id;
						const focus = i === 0 && !showPin;
						return renderProfileButton(p.id, p, disabled, buttonLoading, isPrimary, onProfileSelect, focus);
					})}
			</IntlFormatter>
		</div>
	);
