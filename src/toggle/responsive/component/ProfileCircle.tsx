...
export default class ProfileCircle extends React.Component<ProfileCircleProps, any> {
			...
			<Component className={cx(bem.b({ large }), className)} {...props}>
				<div>
					...
					<div className={bem.e('details')}>
						...
						{isSubprofilesFeatureEnabled && this.renderByline()}
					</div>
				</div>
			</Component>
		);
	...
	private renderCircleBody() {
		...
		// FIXME: should be changed after BE fix the color for primary profile
		const background = isPrimary ? '#f0008c' : backgroundColor;
		disabled = disabled || (primaryDisabled && isPrimary);
		if (loading) {
			return (
				<div style={{ background }} className={bem.e('circle', { loading })}>
		...
		return (
			<div style={{ background }} className={bem.e('circle', { disabled })}>
