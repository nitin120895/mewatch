...
class CollapsibleContainer extends React.PureComponent<CollapsibleContainerProps, CollapsibleContainerState> {
	...
	private showExpanded() {
		const { onShowModal, renderExpanded, className } = this.props;
		const episodeSelector = className.includes('d1-ep-desc');
		if (renderExpanded) {
			onShowModal({
				id: 'expanded',
				...
				componentProps: {
					...
					className: cx(bem.e('modal'), { 'episode-selector': episodeSelector })
				}
			});
		} else {
	...
	render() {
		const { className, ariaLabel, tagName, children, focusable, renderExpanded, episode } = this.props;
		...
		return (
			<CTAWrapper type={CTATypes.Synopsis} data={{ item: episode, isCollapsed: collapsed }}>
				<IntlFormatter
					elementType={tagName}
