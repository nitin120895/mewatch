...
import D2EpisodeList, { D2EpisodeListProps } from 'ref/responsive/pageEntry/itemDetail/d2/D2EpisodeList';
...
const mockEntry: D2EpisodeListProps = {
	id: 'test',
	item: undefined,
	title: '',
	template: 'D2',
...
export default class D2EpisodeListContainer extends React.Component<any, any> {
	constructor(props) {
		super(props);
		this.state = {
			item: undefined,
	...
	private onChangeEpisodeThumbnail = e => {
		const episodeThumbnail = e.target.checked;
		this.setState({
			episodeThumbnail
		});
	};
	...
	private renderRow(item: api.ItemDetail) {
		...
		const { episodeThumbnail, seasonDescription, seasonOrder } = this.state;
		const itemDetailCache = normalizeItemDetail({}, { entries: [], item: item.show } as api.Page);
		const props: D2EpisodeListProps = Object.assign(
			{},
			mockEntry,
					...
					<div>
						<label className="label-inline">
							<input
								type="checkbox"
								name="description"
								checked={episodeThumbnail}
								onChange={this.onChangeEpisodeThumbnail}
							/>
							<span>Display Thumbnails</span>
						</label>
						<label className="label-inline">
							<input
								type="checkbox"
