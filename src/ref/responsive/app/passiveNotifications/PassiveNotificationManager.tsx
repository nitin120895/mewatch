...
> {
	...
	componentDidUpdate(prevProps, prevState: PassiveNotificationManagerState) {
		if (!prevState.notificationOnShow && this.state.notificationOnShow && !this.props.modalVisible) {
			this.startTimeout();
		}
	}
	...
	render() {
		...
		const isContainerHidden = !isClosing && !notificationOnShow && modalVisible;
