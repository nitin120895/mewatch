...
export class DropSelect extends React.Component<DropSelectProps, DropSelectState> {
	...
	private onSelect = (option: DropSelectOption) => {
		this.closeDropDown();
		this.props.onSelect(option.key, this.props.name);
	};
	...
	private closeDropDown = (event?) => {
		...
		if (
			event &&
			(event.target === this.currentOptionElement || this.currentOptionElement.contains(event.target)) &&
			this.state.active
		) {
	...
	private renderDesktopDropDown() {
		const { options, defaultOption, useTranslations, label } = this.props;
