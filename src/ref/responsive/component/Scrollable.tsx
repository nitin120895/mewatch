...
import { BREAKPOINT_RANGES, NUM_COLUMNS } from '../util/grid';
...
export default class Scrollable extends React.Component<ScrollableProps, ScrollableState> {
	...
	componentDidMount() {
		...
		if (this.scrollArea) this.scrollArea.addEventListener('transitionend', this.onScrollAreaTransitioned, false);
	}
	...
	componentDidUpdate(prevProps: ScrollableProps, prevState: ScrollableState) {
		...
		const { onScroll, length } = this.props;
	...
	componentWillUnmount() {
		if (this.scrollArea) {
			this.scrollArea.removeEventListener('touchstart', this.onTouchStart);
			this.scrollArea.removeEventListener('touchmove', this.onTouchMove, { passive: false } as any);
			this.scrollArea.removeEventListener('touchend', this.onTouchEnd);
		}
	...
	public restoreScrollPosition(targetTransformPosition: number) {
		if (targetTransformPosition > 0) {
			const length = this.props.length;
			if (!length) return;

			const newState: any = {};
			newState.maxTransformPosition = this.getMaxTransformPosition();
			// if target position is higher than maximum position, set it to 0
			// it is possible with user lists, when next items page loading performs
			newState.transformPosition =
				targetTransformPosition > newState.maxTransformPosition ? 0 : targetTransformPosition;
			newState.currentIndex = this.recalculateCurrentIndex(newState.transformPosition);
			...
			if (newState.transformPosition === newState.maxTransformPosition) {
				this.onScrollAreaTransitioned(undefined);
			}
		}
	}
	...
	private stopEventPropagation(e: any) {
		if (e && this.props.stopEventPropagation) {
			e.preventDefault();
			e.stopPropagation();
	...
	private onScrollAreaReference = ref => {
		...
		if (this.scrollArea) {
			...
			this.scrollArea.addEventListener('touchmove', this.onTouchMove, { passive: false } as any);
			this.scrollArea.addEventListener('touchend', this.onTouchEnd);
		}
	};

	private onScrollAreaTransitioned = (e: any) => {
	...
	private onTouchEnd = (e: any) => {
		...
		if (this.startPacklistScrolling) {
			...
			const newTransformIndex = this.determineTouchEndTargetElementIndex(speed, this.state.transformPosition);

			const newState = this.scrollToElement(newTransformIndex);
			this.setState(newState);
		}
	...
	render() {
		const { className, children, nextBtnAriaLabel, previousBtnAriaLabel, arrowClassname } = this.props;
		...
		const rightArrowVisible = showArrows && overflows;
