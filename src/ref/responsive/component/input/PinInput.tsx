...
interface PinInputProps {
	...
	required?: boolean;
	disabled?: boolean;
	// this property means visibility of pin input component as part of some containers or accordion items
	isVisible?: boolean;
}
...
export default class PinInput extends React.Component<PinInputProps, PinInputState> {
	...
	componentDidUpdate(prevProps: PinInputProps) {
		const { focusOnUpdate } = this.props;
		if (focusOnUpdate !== prevProps.focusOnUpdate && focusOnUpdate) {
	...
	private onInput = (e, index: number) => {
		...
		digits[index] = !Number.isNaN(value) && !this.focusChangedByRemove ? value : undefined;
	...
	private onClickBtn = e => {
		...
		this.setState((previousState, currentProps) => {
			return { revealed: !previousState.revealed };
		});
	};
	...
	private renderViewLink() {
		...
		const label = revealed ? '@{form_common_input_hide_label|hide}' : '@{form_common_input_show_label|show}';
		return (
			<IntlFormatter
				elementType="button"
				type={'button'}
				className={bem.e('view-pin')}
				disabled={disabled}
